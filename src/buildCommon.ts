...
import { HtmlDomNode, DomSpan, SvgSpan, CssStyle } from "./domTree";
 ...
 */
const mathsym = function(value: string, mode: Mode, options: Options, classes?: string[] = []): SymbolNode {
 ...
 */
const makeOrd = function(group: ParseNode<NODETYPE>, options: Options, type: "mathord" | "textord"): HtmlDocumentFragment | SymbolNode {
 ...
 */
const makeFragment = function(children: HtmlDomNode[]): HtmlDocumentFragment {
 ...
 * and styles
 */
const wrapFragment = function(group: HtmlDomNode, options: Options): HtmlDomNode {
...
const retrieveTextFontName = function(fontFamily: string, fontWeight: string, fontShape: string): string {
    let baseFontName = "";
    switch (fontFamily) {
        case "amsrm":
            baseFontName = "AMS";
...
const svgData: {
    [K in string]: [string, number, number];
} = {
     //   path, width, height
