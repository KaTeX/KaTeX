...
export const buildExpression = function(
    ...
    surrounding: [DomType | null, DomType | null] = [null, null]
): HtmlDomNode[] {
...
const traverseNonSpaceNodes = function(
    nodes: HtmlDomNode[],
    callback: ((htmlDomNode: HtmlDomNode, htmlDomNode: HtmlDomNode) => HtmlDomNode | null),
    prev: {
        node: HtmlDomNode,
        insertAfter?: ((htmlDomNode: HtmlDomNode) => void)
    },
    next: HtmlDomNode | null,
    isRoot: boolean,
) {
...
const checkPartialGroup = function(node: HtmlDomNode): DocumentFragment<HtmlDomNode> | Anchor | DomSpan | null {
...
const getOutermostNode = function(node: HtmlDomNode, side: Side): HtmlDomNode {
    const partialGroup = checkPartialGroup(node);
    if (partialGroup) {
...
export const getTypeOfDomTree = function(node: HtmlDomNode | null, side: Side | null): DomType | null {
    if (!node) {
        return null;
    }
    if (side) {
 ...
 */
export const buildGroup = function(group: AnyParseNode | null, options: Options, baseOptions?: Options): HtmlDomNode {
    if (!group) {
        return makeSpan();
    }
