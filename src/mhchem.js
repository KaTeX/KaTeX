 ...
 *    5. Code is reformatted to use 4 space indents and 80 char max per line.
 *    6. Many minor edits were made to pass the linter.
 *
    ...
    go: function(input, stateMachine) {
        ...
        while (true) {
            ...
            iterateTransitions: for (let i = 0; i < t.length; i++) {
                const matches = mhchemParser.patterns.match_(t[i].pattern, input);
                if (matches) {
                    //
                    // Execute actions
                    //
                    ...
                    for (let iA = 0; iA < task.action_.length; iA++) {
                        ...
                        if (machine.actions[task.action_[iA].type_]) {
                            ...
                            throw new ParseError("mhchem bug A. Please report. ("
            ...
            if (watchdog <= 0) {
                ...
                throw new ParseError("mhchem bug U. Please report.");
            }
        }
    },
...
// eslint-disable-next-line transform-runtime-aliasing/no-transform-runtime-aliasing
    ...
    patterns: {
        //
        // Matching patterns either regexps or
        // function that return null or {match_:"a", remainder:"bc"}
        //
        patterns: {
            // property names must not look like integers ("2") for correct
            // property traversal order, later on
            empty: /^$/,
            else: /^./,
            else2: /^./,
            space: /^\s/,
            ...
            "state of aggregation $": function(input) {
                ...
                if (a && a.remainder.match(/^($|[\s,;)\]}])/)) {
                 ...
                 // OR crystal system ($o$) (\ca$c$)
                const m = input.match(/^(?:\((?:\\ca\s?)?\$[amothc]\$\))/);
                if (m) {
                    return {
                        match_: m[0],
                        remainder: input.substr(m[0].length),
            ...
            "\\ca": /^\\ca(?:\s+|(?![a-zA-Z]))/,
            "\\x": /^(?:\\[a-zA-Z]+\s*|\\[_&{}%])/,
            // only those with numbers in front, because the others will be
            // formatted correctly anyway
            ...
            amount: function(input) {
                let match;
                // e.g. 2, 0.5, 1/2, -2, n/2, +;
                // $a$ could be added later in parsing
                match = input.match(
                    // eslint-disable-next-line max-len
            ...
            const _findObserveGroups = function(input, i, endChars) {
                ...
                while (i < input.length) {
                    ...
                    if (match !== null && braces === 0) {
                        ...
                        if (braces === 0) {
                            throw new ParseError("Extra close brace or " +
                                "missing open brace");
                        } else {
                            braces--;
                        }
        ...
        //
        // Matching function
        // e.g. match("a", input) will look for the regexp called "a" and
        // see if it matches
        ...
        match_: function(m, input) {
            ...
            if (pattern === undefined) {
                ...
                throw new ParseError("mhchem bug P. Please report. (" + m + ")");
            } else if (typeof pattern === "function") {
                // cannot use cached var pattern here, because some pattern
                // functions need this===mhchemParser
    ...
    //
    // createTransitions
    // convert  { 'letter': { 'state': { action_: 'output' } } }  to
    // { 'state' =>
    //     [ { pattern: 'letter', task: { action_: [{type_: 'output'}] } } ]
    // }
        ...
        for (pattern in o) {
            if (Object.prototype.hasOwnProperty.call(o, pattern)) {
                for (state in o[pattern]) {
                    if (Object.prototype.hasOwnProperty.call(o[pattern], state)) {
            ...
            if (Object.prototype.hasOwnProperty.call(o, pattern)) {
                for (state in o[pattern]) {
                    if (Object.prototype.hasOwnProperty.call(o[pattern], state)) {
                        ...
                        for (i = 0; i < stateArray.length; i++) {
                            //
                            // 2a. Normalize actions into array:
                            //     'text=' ==> [{type_:'text='}]
                            // (Note to myself: Resolving the function here would be
                            //  problematic. It would need .bind (for *this*) and
                            //  currying (for *option*).)
                            //
                            ...
                            for (let j = 0; j < patternArray.length; j++) {
                                if (stateArray[i] === "*") {
                                    ...
                                    for (const t in transitions) {
                                        if ({}.hasOwnProperty.call(transitions,
                                            t)) {
                                            transitions[t].push({
    ...
    ce: {
        ...
        actions: {
            ...
            output: function(buffer, m, entityFollows) {
                // entityFollows:
                //   undefined = if we have nothing else to output, also ignore
                //   the just read space (buffer.sb)
                //   1 = an entity follows, never omit the space if there was
                //       one just read before (can only apply to state 1)
                //   2 = 1 + the entity can have an amount, so output a\,
                //       instead of converting it to o
                //       (can only apply to states a|as)
                let ret;
                if (!buffer.r) {
                    ret = [];
                    if (
        ...
        actions: {
            output: function(buffer) {
                if (buffer.text_) {
                    ...
                    for (const p in buffer) {
                        if (Object.prototype.hasOwnProperty.call(buffer, p)) {
                            delete buffer[p];
                        }
                    }
                    return ret;
    ...
    _go2: function(buf) {
        ...
        switch (buf.type_) {
            ...
            case "bond":
                ...
                if (!res) {
                    throw new ParseError("mhchem Error. Unknown bond type (" +
                        buf.kind_ + ")");
                }
                break;
            case "frac":
            ...
            case "arrow":
                ...
                arrow = "\\x" + texify._getArrow(buf.r);
                if (b6.rq) {
                    arrow += "[{" + b6.rq + "}]";
                }
                if (b6.rd) {
            ...
            default:
                ...
                throw new ParseError("mhchem bug T. Please report.");
        }
        assertString(res);
        return res;
    },
    ...
    _getBond: function(a) {
        switch (a) {
            ...
            case "~":
                return "{\\tripledash}";
            case "~-":
                return "{\\mathrlap{\\raisebox{-.1em}{$-$}}" +
                        "\\raisebox{.1em}{$\\tripledash$}}";
            case "~=":
                return "{\\mathrlap{\\raisebox{-.2em}{$-$}}" +
                        "\\mathrlap{\\raisebox{.2em}{$\\tripledash$}}-}";
            case "~--":
                return "{\\mathrlap{\\raisebox{-.2em}{$-$}}" +
                        "\\mathrlap{\\raisebox{.2em}{$\\tripledash$}}-}";
            case "-~-":
