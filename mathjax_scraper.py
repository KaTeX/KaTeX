...
all_matches = re.findall("\\\\.</span>|\\\\[A-z]*</span>", res_text[start_index:end_index])
...
old_text = readme_text[
    readme_text.find(mathjax_section_start) + len(mathjax_section_start):readme_text.find(mathjax_section_end)
]
...
for key in list(string.ascii_lowercase) + ['basic']:
    section = ''
    section_header = '| **{}** | |\n'.format(key.upper())
    section += section_header
    for name in alpha_dict[key]:
        # TODO: checkbox goes on the right side
        section += '| {} | {} |\n'.format(name, '')
    table += section
