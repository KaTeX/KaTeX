...
- `siunitx`: `string` (default: `""`). Default siunitx configuration, equivalent to prepending `\sisetup{...}` at the start of each render call. This option requires loading the siunitx extension (`katex/contrib/siunitx`) first. Example: `siunitx: "group-separator={,},group-minimum-digits=4,per-mode=reciprocal"`.
