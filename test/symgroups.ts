...
function typeset() {
    const proc = childProcess.spawn(
        ...
        {stdio: "ignore"});
    proc.on("exit", function(code: number | null, signal: string | null) {
        if (signal) {
...
function evaluate(err: Error | null, log: string) {
    if (err) {
        throw err;
    }
    ...
    while ((match = reMM.exec(log)) !== null) {
        const list = match[1];
        const partsMatch = reParts.exec(list);
        if (!partsMatch) {
            const emptyMatch = reEmpty.exec(list);
            if (emptyMatch) {
        ...
        const idx = extractDigits(partsMatch[1]);
        const atom = partsMatch[2];
        const katexType = types[extractDigits(partsMatch[3]) - 1] || "???";
        const atomMatch = reAtom.exec(atom);
        if (!atomMatch) {
