...
If all you want is to recreate the Firefox and Chromium snapshots, then
you can do so by:

    yarn test:screenshots:update

It will fetch the required Selenium Docker images, and use them to take
screenshots. Safari screenshots are generated by the GitHub Actions
macOS screenshotter job because exact comparisons depend on the macOS
and Safari versions.
