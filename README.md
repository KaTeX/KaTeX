...
- `allowedProtocols`: `string[]` or `boolean`. Allowed protocols in `\href`. Use `"_relative"` to allow relative urls, and `true` or `false` to allow or disallow all protocols. (default: `["http", "https", "mailto", "_relative"]`)
