...
However, the message content of KaTeX exceptions (which repeats some of the
LaTeX source code) should not be considered safe to render, so you should
convert `&`, `<`, `>` characters to `&amp;`, `&lt;`, `&gt;`
(e.g., using `_.escape`) before including LaTeX source code or any
exceptions encountered in your HTML/DOM.
