...
- `allowAllSymbols`: `boolean`. If `true`, *all* unicode characters will be allowed in math and text modes; you will likely get warnings about missing character metrics, and layouts may be incorrect, but KaTeX will at least try to render something instead of throwing an error. This option also causes a true setting of `unicodeTextInMathMode`. The default is `false`, which follows KaTeX's more conservative character set.
